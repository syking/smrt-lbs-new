...
dojo.declare('esri.ux.layers.ClusterLayer', esri.layers.GraphicsLayer, {
    ...
    clusterFeatures: function(redraw) {
        ...
        var width = levelResolution * tileInfo.width * 1.2;
        var height = levelResolution * tileInfo.height * 1.2;
                        ...
                        if ((colIndex >= minColIdx) & (colIndex <= maxColIdx)) {
                            ...
                            if (col.length > 2) { //clustered graphic
                                ...
                                dojo.forEach(col, function(point) {
                                    ...
                                    var sbl;
                                    if(point.attributes.techName == 'event-idle'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/blue-dot.png', 32, 32);
                                    }else if(point.attributes.techName == 'event-speeding'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/green-dot.png', 32, 32);
                                    }else if(point.attributes.techName == 'event-sudden-acceleration'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/ltblue-dot.png', 32, 32);
                                    }else if(point.attributes.techName == 'event-sudden-braking'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/pink-dot.png', 32, 32);
                                    }else if(point.attributes.techName == 'event-sudden-left'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/purple-dot.png', 32, 32);
                                    }else if(point.attributes.techName == 'event-sudden-right'){
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/red-dot.png', 32, 32);
                                    }
                                    else{
                                        sbl = new esri.symbol.PictureMarkerSymbol('/public/images/yellow-dot.png', 32, 32);
                                    }
                                    ...
                                    var textSbl = new esri.symbol.TextSymbol(point.attributes.name);
                                    textSbl.setOffset(0,15);

                                    var textGra = new esri.Graphic(point, textSbl, point.attributes, this._infoTemplate);

                                    if(point.attributes.activeStatus == 'on'){
                                        vehicleGra.visible = true;
                                        textGra.visible = true;
                                    }
                                    else{
                                        vehicleGra.visible = false;
                                        textGra.visible = false;
                                    }

                                    if(!map.showTextTooltip){
                                        textGra.visible = false;
                                    }

                                    this.add(vehicleGra);
                                    map.tooltipLayer.add(textGra);

                                    if(map.infoWindow.isShowing && undefined != map.selectedGraphic &&
                                        point.attributes.name == map.selectedGraphic.attributes.name){
