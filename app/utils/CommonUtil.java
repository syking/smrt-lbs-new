...
public class CommonUtil {
	...
	/**
	 * 组装KendoGrid需要的data
	 * @param models models Model list
	 * @param unless unless 哪些字段不需要显示的
	...
	public static List<Map> assemColumns(Class<?> model,Map<Class<?>, String> pojos, String... unless){
		...
		label: for (Field f : fields){
			...
			if (pojos != null && !pojos.isEmpty() && pojos.containsKey(f.getType()))
				name = pojos.get(f.getType());
			
			Map map = new HashMap();
			
			map.put("field", name);
			map.put("title", Splitter.init(name).byUpperCase().by("_", ".").connect(" ").split().upperFirst().toString());
